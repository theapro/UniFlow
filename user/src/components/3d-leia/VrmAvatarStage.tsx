...
export function VrmAvatarStage(props: {
  ...
  useEffect(() => {
    ...
    // Subtle halo effect (separate from the Voice page spheres)
    const haloGroup = new THREE.Group();
    haloGroup.position.set(0, 1.26, -0.6);
    scene.add(haloGroup);
    ...
    const nextVowelAtRef = { v: 0 };
    const vowelIndexRef = { v: 0 };
    ...
    (async () => {
      try {
        ...
        // Slightly lower the model to center it in the sphere mask.
        ...
        // This is intentionally subtle but strong enough to avoid the "T" silhouette.
        const ARM_DOWN = 1.15;
    ...
    const updateLipSync = (t: number, speak: number) => {
      ...
      if (!speaking || speak < 0.02) {
        for (const [vrm0, vrm1] of visemes) {
          safeSetExpression(vrm, vrm0, 0);
          safeSetExpression(vrm, vrm1, 0);
        }
        return;
      }

      // Pick a vowel every ~120-220ms for basic realism.
      if (t >= nextVowelAtRef.v) {
        vowelIndexRef.v = (vowelIndexRef.v + 1) % visemes.length;
        nextVowelAtRef.v = t + 0.12 + Math.random() * 0.1;
      }
      ...
      const [vrm0, vrm1] = visemes[vowelIndexRef.v];
      safeSetExpression(vrm, vrm0, mouth);
      safeSetExpression(vrm, vrm1, mouth);
    };
  ...
  }, [modelUrl, visemes, speakingLevelRef, listeningLevelRef]);
