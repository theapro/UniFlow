...
function isSecureMicContext(): boolean {
  ...
  // `isSecureContext` covers localhost too in modern browsers.
...
export function useVoiceChat(options: UseVoiceChatOptions = {}) {
  ...
  const speakTextFallback = useCallback(async (text: string) => {
    ...
    const cleaned = stripThinkBlocks(String(text ?? "")).trim();
    if (!cleaned) return false;

    const synth = window.speechSynthesis;
    const Utter = (window as any).SpeechSynthesisUtterance as
  ...
  const sendBlob = useCallback(
    async (blob: Blob, params: { sessionId?: string; chatModel?: string }) => {
      ...
      // Heuristic guard: very tiny blobs are usually empty/stopped instantly.
      // Keep this low to avoid rejecting short utterances.
      if (blob.size < 256) {
      ...
      try {
        ...
        const text = stripThinkBlocks(
          String(json?.text ?? json?.replyText ?? ""),
        ).trim();
        ...
        // Text-only fallback is allowed (e.g., backend TTS is unavailable).
        if (result.audioBase64) {
          await playAudio(result);
        } else {
          await speakTextFallback(result.text);
        }
    ...
    [endpoint, playAudio, speakTextFallback],
  );
  ...
  const start = useCallback(async () => {
    ...
    const mrCtor = (window as any).MediaRecorder as
      | typeof MediaRecorder
      | undefined;

    if (!mrCtor) {
      const msg = "This browser does not support microphone recording";
      setError(msg);
      onErrorRef.current?.(msg);
      snapshotDebug("mediarecorder:missing");
      return;
    }
    ...
    if (!navigator?.mediaDevices?.getUserMedia) {
      ...
      setError(msg);
      onErrorRef.current?.(msg);
      snapshotDebug("getUserMedia:missing");
      return;
    }
    ...
    if (!isSecureMicContext()) {
      ...
      setError(msg);
      onErrorRef.current?.(msg);
      snapshotDebug("getUserMedia:insecureContext");
      return;
    }
    ...
    try {
      ...
      try {
        recorder = new mrCtor(stream, mimeType ? { mimeType } : {});
      } catch {
        // Fallback: some platforms throw even when isTypeSupported returns true.
        recorder = new mrCtor(stream);
      }
