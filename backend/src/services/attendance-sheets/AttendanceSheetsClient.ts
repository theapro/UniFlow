...
export type AttendanceSheetsClientOptions = {
...
export class AttendanceSheetsClient {
  private readonly spreadsheetId: string;
  private readonly sheets = google.sheets("v4");
  private readonly auth;
  private sheetIdByTitleCache: Map<string, number> | null = null;

  constructor(opts?: Partial<AttendanceSheetsClientOptions>) {
    const spreadsheetId =
      opts?.spreadsheetId ?? env.attendanceSheetsSpreadsheetId ?? "";
    ...
    if (!clientEmail) throw new Error("ATTENDANCE_SHEETS_MISSING_CLIENT_EMAIL");
    if (!privateKey) throw new Error("ATTENDANCE_SHEETS_MISSING_PRIVATE_KEY");
  ...
  async batchUpdateA1Values(opts: {
    data: Array<{ range: string; value: string }>;
    valueInputOption?: "RAW" | "USER_ENTERED";
  }): Promise<void> {
    const data = (opts.data ?? []).map((d) => ({
      range: d.range,
      values: [[d.value]],
    }));
    if (data.length === 0) return;
