...
import { GradesSheetsOverview } from "./GradesSheetsOverview";
import { GradesSheetsPreview } from "./GradesSheetsPreview";

export function GradesSheetsTabs({ dict }: { dict: any }) {
  ...
  const { data: status, isLoading: statusLoading } = useQuery({
    ...
    queryFn: () => gradesSheetsApi.status().then((r) => r.data.data as any),
    refetchInterval: 15_000,
  });

  const { data: tabs } = useQuery({
    queryKey: ["grades-sheets", "tabs"],
    queryFn: () => gradesSheetsApi.tabs().then((r) => r.data.data as any),
    refetchInterval: 60_000,
  });
