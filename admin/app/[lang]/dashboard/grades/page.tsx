  ...
  if (!metaRes.ok) {
    return (
      <div className="container max-w-7xl py-10 space-y-12">
        <PageHeader
          title={title}
          description="Browse groups and open one to manage grades"
        />
  ...
  if (sections.length === 0) {
    return (
      <div className="container max-w-7xl py-10 space-y-12">
        <PageHeader
          title={title}
          description="Pick a subject, then open a group to manage grades"
        />
                      ...
                      {s.groups.length ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                          {s.groups.map((g) => (
                            <Link
                              key={g.id}
                              href={`/${lang}/dashboard/grades/${g.id}?subjectId=${encodeURIComponent(
                                s.subject.id,
                              )}`}
                              className={cn(
